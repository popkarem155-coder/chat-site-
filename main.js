...
const STORAGE_KEYS = { state: "kareem3_state_v2", currentSession: "kareem3_session_v2", };

const CONFIG = { PUBLIC_MESSAGE_CAP: 70, PRIVATE_MESSAGE_CAP: 70, MAX_NOTIFICATIONS: 70, SESSION_TTL_MS: 1000 * 60 * 60 * 12, TOAST_MS: 2200, DEMO_REPLY_DELAY_MS: 900, };

const els = {}; const state = { ready: false, view: "home", menuOpen: false, privateDrawerOpen: false, currentAccountId: null, selectedUserId: null, selectedPrivatePeerId: null, searchQuery: "", privateSearchQuery: "", accounts: [], publicMessages: [], privateThreads: {}, toastHostEl: null, menuDrawerOverlay: null, privateDrawerOverlay: null, menuDrawer: null, privateDrawerEl: null, monitorPanelEl: null, bridge: null, bridgeStatus: null, activitySaveTimer: null, pullRefresh: { tracking: false, startY: 0, startX: 0 }, };
...
function clampText(value, maxLen) { return normalizeText(value).slice(0, maxLen); }

function createId(prefix = "id") { return ${prefix}_${Math.random().toString(36).slice(2, 10)}_${Date.now().toString(36)}; }

function hashString(str) { let hash = 0; const input = String(str || ""); for (let i = 0; i < input.length; i += 1) { hash = (hash << 5) - hash + input.charCodeAt(i); hash |= 0; } return Math.abs(hash); }

function formatTime(ts) { const d = new Date(Number(ts || now())); return new Intl.DateTimeFormat("ar-EG", { hour: "2-digit", minute: "2-digit", }).format(d); }

function timeAgo(ts) { const diff = Math.max(0, now() - Number(ts || 0)); const sec = Math.floor(diff / 1000); if (sec < 10) return "الآن"; if (sec < 60) return منذ ${sec} ث; const min = Math.floor(sec / 60); if (min < 60) return منذ ${min} د; const hr = Math.floor(min / 60); if (hr < 24) return منذ ${hr} س; const day = Math.floor(hr / 24); return منذ ${day} ي; }

function durationLabel(ms) { const totalMinutes = Math.floor(Math.max(0, Number(ms || 0)) / 60000); const hours = Math.floor(totalMinutes / 60); const minutes = totalMinutes % 60;
...
function getThreadKey(a, b) { return [String(a), String(b)].sort().join("__"); }

function normalizeAccount(acc) { if (!acc || typeof acc !== "object") return null;
...
function normalizeThread(thread) { if (!thread || typeof thread !== "object") return null; const participants = Array.isArray(thread.participants) ? thread.participants.map(String) : []; const messages = Array.isArray(thread.messages) ? thread.messages.map(normalizeMessage).filter(Boolean) : []; return { participants, messages, updatedAt: Number(thread.updatedAt || 0), }; }

function normalizeMessage(message) { if (!message || typeof message !== "object") return null; return { id: String(message.id || createId("msg")), senderId: message.senderId == null ? null : String(message.senderId), senderLabel: clampText(message.senderLabel || "مستخدم", 50) || "مستخدم", text: clampText(message.text || "", 500), at: Number(message.at || now()), }; }

function getAccounts() { return Array.isArray(state.accounts) ? state.accounts : []; }

function setAccounts(list) { state.accounts = Array.isArray(list) ? list.map(normalizeAccount).filter(Boolean) : []; }

function getAccountById(id) { if (!id) return null; return getAccounts().find((acc) => acc.id === String(id)) || null; }

function getAccountByUsername(username) { const q = normalizeText(username).toLowerCase(); if (!q) return null; return getAccounts().find((acc) => normalizeText(acc.username).toLowerCase() === q) || null; }

function getCurrentAccount() { return getAccountById(state.currentAccountId); }

function isSessionExpired(session) { if (!session) return true; return Number(session.expiresAt || 0) > 0 && now() > Number(session.expiresAt || 0); }

function getCurrentSession() { try { const raw = localStorage.getItem(STORAGE_KEYS.currentSession); if (!raw) return null; const parsed = JSON.parse(raw); if (!parsed || typeof parsed !== "object") return null; return { accountId: String(parsed.accountId || ""), startedAt: Number(parsed.startedAt || 0), expiresAt: Number(parsed.expiresAt || 0), }; } catch { return null; } }

function saveCurrentSession(session) { try { localStorage.setItem(STORAGE_KEYS.currentSession, JSON.stringify(session)); } catch {} }

function readStorage() { try { const raw = localStorage.getItem(STORAGE_KEYS.state); if (!raw) return false; const parsed = JSON.parse(raw); if (!parsed || typeof parsed !== "object") return false;
...
function saveStorage() { const payload = { accounts: getAccounts(), publicMessages: Array.isArray(state.publicMessages) ? state.publicMessages.slice(-CONFIG.PUBLIC_MESSAGE_CAP) : [], privateThreads: state.privateThreads || {}, currentAccountId: state.currentAccountId, searchQuery: state.searchQuery, privateSearchQuery: state.privateSearchQuery, view: state.view, selectedUserId: state.selectedUserId, selectedPrivatePeerId: state.selectedPrivatePeerId, };
...
function ensureDemoUsers() { if (getAccounts().length) return;
...
function ensureCurrentAccount() { const session = getCurrentSession(); if (!session || isSessionExpired(session)) { state.currentAccountId = null; return null; }
...
function ensureWelcomePublicMessage() { if (Array.isArray(state.publicMessages) && state.publicMessages.length) return;
...
function getThread(a, b, createIfMissing = false) { if (!a || !b) return null;
...
function prunePublicMessages() { if (!Array.isArray(state.publicMessages)) state.publicMessages = []; if (state.publicMessages.length > CONFIG.PUBLIC_MESSAGE_CAP) { state.publicMessages = state.publicMessages.slice(-CONFIG.PUBLIC_MESSAGE_CAP); } }

function prunePrivateThreads() { const cleaned = {};
...
function getPrivateChatsForCurrentUser() { const current = getCurrentAccount(); if (!current) return [];
...
function getPrivateChatMatches(query = state.privateSearchQuery) { const current = getCurrentAccount(); if (!current) return [];
...
function getUnreadNotificationCount() { const current = getCurrentAccount(); if (!current || !Array.isArray(current.notifications)) return 0; return current.notifications.filter((n) => !n.read).length; }

function getMonitorItems() { const current = getCurrentAccount(); if (!current || !Array.isArray(current.notifications)) return []; return [...current.notifications].sort((a, b) => Number(b.at) - Number(a.at)); }

function notifyProfileViewed(targetAccountId, viewerLabel, viewerId = null) { const target = getAccountById(targetAccountId); if (!target) return; if (viewerId && viewerId === targetAccountId) return;
...
function addPrivateMessage(peerId, text, senderId = null, senderLabel = "مستخدم") { const current = getCurrentAccount(); if (!current || !peerId) return null;
...
function scheduleDemoReply(peerId, originalText) { const peer = getAccountById(peerId); if (!peer || !peer.isDemo) return;
...
function setBridgeUser() { const bridge = state.bridge || window.KAREEM3_DB; if (!bridge || typeof bridge.setUser !== "function") return;
...
function softRefresh() { readStorage(); ensureCurrentAccount(); ensureDemoUsers(); ensureWelcomePublicMessage(); prunePublicMessages(); prunePrivateThreads(); saveStorage(); renderAll(); setBridgeUser(); }

function showToast(message) { if (!state.toastHostEl) { state.toastHostEl = document.createElement("div"); state.toastHostEl.id = "toastHost"; state.toastHostEl.className = "toast-host"; document.body.appendChild(state.toastHostEl); }
...
function focusInput(input) { if (!input) return; window.requestAnimationFrame(() => { try { input.focus({ preventScroll: true }); } catch { input.focus(); }
...
function makeOverlay(id, zIndex) { let overlay = $(id); if (!overlay) { overlay = document.createElement("div"); overlay.id = id; document.body.appendChild(overlay); }
...
function setOverlayVisible(overlay, visible) { if (!overlay) return; overlay.style.display = visible ? "block" : "none"; }

function updateBodyScrollLock() { const locked = state.menuOpen || state.privateDrawerOpen;

document.body.style.overflow = locked ? "hidden" : "";

}

function positionDrawerSide(drawer, side = "right") { if (!drawer) return;
...
function ensurePrivateDrawer() { if (state.privateDrawerEl) return state.privateDrawerEl;
...
function ensureMonitorPanel() { if (state.monitorPanelEl) return state.monitorPanelEl; if (!els.menuDrawer) return null;
...
function openMenuDrawer() { if (!els.menuDrawer) return; ensurePrivateDrawer(); closePrivateDrawer(); state.menuOpen = true; els.menuDrawer.classList.remove("is-hidden"); els.menuDrawer.setAttribute("aria-hidden", "false"); if (els.menuDrawerOverlay) setOverlayVisible(els.menuDrawerOverlay, true); updateBodyScrollLock(); }

function closeMenuDrawer() { if (!els.menuDrawer) return; state.menuOpen = false; els.menuDrawer.classList.add("is-hidden"); els.menuDrawer.setAttribute("aria-hidden", "true"); if (els.menuDrawerOverlay) setOverlayVisible(els.menuDrawerOverlay, false); updateBodyScrollLock(); }

function openPrivateDrawer() { const drawer = ensurePrivateDrawer(); if (!drawer) return;
...
function closePrivateDrawer() { const drawer = state.privateDrawerEl; if (!drawer) return;
...
function setView(viewName) { state.view = viewName;
...
function openHome() { state.selectedUserId = null; state.selectedPrivatePeerId = null; setView("home"); renderHomeView(); }

function openProfile() { if (!canUseCurrentSession()) return;
...
function openUserProfileById(userId) { if (!userId) return;
...
function openPrivateChat(peerId, silent = false) { const peer = getAccountById(peerId);
...
function openMonitorPanel() { if (!canUseCurrentSession()) return;
...
function markCurrentNotificationsRead() { const current = getCurrentAccount(); if (!current || !Array.isArray(current.notifications)) return;
...
function loginAccount(account) { if (!account) return;
...
function commitCurrentSession(force = false) { const acc = getCurrentAccount(); const session = getCurrentSession();
...
function logoutCurrentAccount(showMessage = true) { commitCurrentSession(true); state.selectedPrivatePeerId = null; state.selectedUserId = null; ensureCurrentAccount(); saveStorage(); setBridgeUser(); renderAll();

if (showMessage) showToast("تم تسجيل الخروج.");

}

function canUseCurrentSession() { const acc = getCurrentAccount(); const session = getCurrentSession();
...
function markActivity() { const acc = getCurrentAccount(); const session = getCurrentSession();
...
function setupBridge() { const bridge = window.KAREEM3_DB; if (!bridge || typeof bridge.init !== "function") return Promise.resolve(null);
...
function getDisplayModeLabel() { const current = getCurrentAccount(); if (!current) return "زائر"; if (isAccountFeatured(current)) return ${getDisplayName(current)} • مستخدم مميز; if (isAccountOnline(current)) return ${getDisplayName(current)} • متصل الآن; return ${getDisplayName(current)} • غير نشط; }

function renderShellState() { const current = getCurrentAccount();
...
function buildMessageElement(message) { const sender = getAccountById(message.senderId); const senderName = normalizeText( sender ? getDisplayName(sender) : message.senderLabel || "مستخدم" );
...
function renderPublicMessages() { if (!els.publicMessages) return; els.publicMessages.innerHTML = "";
...
function renderOnlineUsers() { if (!els.onlineUsersList || !els.onlineUsersEmpty) return;
...
function renderFeaturedUsers() { if (!els.featuredUsersList || !els.featuredUsersEmpty) return;
...
function renderHomeView() { renderShellState(); renderPublicMessages(); renderOnlineUsers(); renderFeaturedUsers(); renderUserSearchResults(); renderPrivateDrawerChatsList(); }

function renderProfileView() { const current = getCurrentAccount(); if (!current) return;
...
function renderPrivateDrawerChatsList() { if (!els.privateChatsList || !els.privateChatsEmpty) return;
...
function renderPrivateConversation() { if ( !els.privateMessages || !els.privateChatTitle || !els.privateChatMeta || !els.privateChatAvatar ) { return; }
...
function renderUserView() { const target = getAccountById(state.selectedUserId);
...
function renderMonitorPanel() { if (!state.monitorPanelEl) return;
...
function syncUserSearchInputs(value) { const normalized = normalizeText(value || ""); state.searchQuery = normalized;
...
function renderUserSearchResults() { if (!els.userSearchResults || !els.searchResultCount) return;
...
function renderAll() { renderShellState(); renderHomeView(); renderProfileView(); renderPrivateDrawerChatsList(); renderPrivateConversation(); renderUserView(); renderMonitorPanel(); renderUserSearchResults(); positionDrawers(); }

function renderPrivateViewIfNeeded() { if (state.view === "private") { renderPrivateConversation(); } }

function handleProfileSave(event) { event.preventDefault();
...
function handleProfileImagePick(event) { const file = event.target.files?.[0]; if (!file) return;
...
function handlePublicSubmit(event) { event.preventDefault();
...
function handlePrivateSubmit(event) { event.preventDefault();
...
function sendPublicMessage(text, silent = false) { const messageText = normalizeText(text);
...
function sendPrivateMessage(peerId, text, silent = false) { const messageText = normalizeText(text);
...
function handleAppTitleClick() { softRefresh(); }

function handlePrivateShortcutClick() { if (!state.privateDrawerOpen) { openPrivateDrawer(); } else { closePrivateDrawer(); } }

function handleBackFromPrivate() { closePrivateDrawer(); openHome(); }

function attachSendButtonKeyboardProtection() { [els.publicSendBtn, els.privateSendBtn].forEach((btn) => { if (!btn) return; btn.addEventListener("pointerdown", (e) => e.preventDefault()); btn.addEventListener( "touchstart", (e) => e.preventDefault(), { passive: false } ); }); }

function attachPullToRefresh() { const root = els.appMain || els.app || document.body; if (!root) return;
...
function bindEvents() { if (els.menuBtn) { els.menuBtn.addEventListener("click", (event) => { event.stopPropagation(); if (state.menuOpen) { closeMenuDrawer(); } else { openMenuDrawer(); } }); }
...
function positionDrawers() { if (els.menuDrawer) { positionDrawerSide(els.menuDrawer, "right"); }
...
function initInputsText() { if (els.messagesBtn) els.messagesBtn.textContent = "✉"; if (els.menuBtn) els.menuBtn.textContent = "☰"; }

async function init() { if (state.ready) return; state.ready = true;
...
function cacheElements() { els.app = $("app"); els.appHeader = $("appHeader") || $("topbar"); els.appMain = $("appMain") || $("homeView");
...
function handleAppTitleRefresh() { softRefresh(); }

function getThreadMessagesForPeer(peerId) { const current = getCurrentAccount(); if (!current || !peerId) return [];
...
function openAccountProfileById(userId) { if (!userId) return;
...
function renderPrivateChatHelpers() { renderPrivateDrawerChatsList(); renderPrivateConversation(); }

function bindLowLevelAliases() { window.KAREEM3 = { refresh: softRefresh, logout: logoutCurrentAccount, openProfile, openUserProfileById, openAccountProfileById, openPrivateChat, openPrivateDrawer, closePrivateDrawer, openMenuDrawer, closeMenuDrawer, state: () => ({ currentAccount: getCurrentAccount(), currentSession: getCurrentSession(), unreadNotifications: getUnreadNotificationCount(), view: state.view, privateDrawerOpen: state.privateDrawerOpen, menuOpen: state.menuOpen, }), }; }

function isAccountOnline(acc) { if (!acc) return false; const current = getCurrentAccount(); if (current && current.id === acc.id) return true; return Boolean(acc.lastSeenAt && now() - Number(acc.lastSeenAt) < 1000 * 60 * 60 * 2); }

function isAccountFeatured(acc) { if (!acc) return false; return Number(acc.totalActiveMs || 0) >= 1000 * 60 * 60 * 2; }

function getDisplayName(acc) { if (!acc) return "مستخدم"; return normalizeText(acc.profile?.name || acc.username || "مستخدم") || "مستخدم"; }

function getAvatarInitial(acc) { const name = getDisplayName(acc); return name ? name[0] : "؟"; }

function setAvatar(el, acc, fallback = "؟") { if (!el) return; const avatar = acc?.profile?.avatar; if (avatar) { el.style.backgroundImage = url(${avatar}); el.textContent = ""; } else { el.style.backgroundImage = ""; el.textContent = fallback || getAvatarInitial(acc); } }

function getActiveDurationForAccount(acc) { if (!acc) return 0; let total = Number(acc.totalActiveMs || 0); if (state.currentAccountId && state.currentAccountId === acc.id) { const session = getCurrentSession(); if (session && session.accountId === acc.id && !isSessionExpired(session)) { total += Math.max(0, now() - Number(session.startedAt || now())); } } return total; }
