...
const state = { accounts: [], publicMessages: [], privateThreads: {}, currentAccountId: null, selectedPrivatePeerId: null, selectedUserId: null, pendingAction: null, monitorPanelEl: null, toastHostEl: null, activitySaveTimer: null, intervalTimer: null, view: "home", searchQuery: "", externalDB: null, };
...
function openHome() { state.selectedUserId = null; state.selectedPrivatePeerId = state.selectedPrivatePeerId || null; setView("home"); }
...
if (!messageText) {
  if (!silent) showToast("اكتب رسالة أولاً.");
  return false;
}
...
if (!peerId) {
  if (!silent) showToast("اختر شخص أولاً.");
  return false;
}
...
if (!current) {
  ...
  els.privateChatsEmpty.textContent = "لا يوجد حساب نشط حالياً.";
  return;
}
...
if (!current) {
  ...
  placeholder.textContent = "لا يوجد حساب نشط حالياً.";
...
function handlePublicSubmit(event) { event.preventDefault(); const text = normalizeText(els.publicMessageInput?.value || ""); if (!text) { showToast("اكتب رسالة أولاً."); return; }
  ...
  state.publicMessages = [
    {
      ...
      text: "أهلاً بك في شات نار. جرّب اكتب رسالة.",
      at: now() - 5 * 60 * 1000,
    },
  ];
}
