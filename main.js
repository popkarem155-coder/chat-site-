...
const KEYS = { accounts: "kareem3_accounts", publicMessages: "kareem3_publicMessages", privateThreads: "kareem3_privateThreads", currentSession: "kareem3_currentSession", guestSeed: "kareem3_guestSeed", };

const CONFIG = { SESSION_TTL_MS: 24 * 60 * 60 * 1000, ONLINE_WINDOW_MS: 15 * 60 * 1000, FEATURED_WINDOW_MS: 2 * 60 * 60 * 1000, PUBLIC_MESSAGE_CAP: 70, MAX_NOTIFICATIONS: 20, TOAST_MS: 2400, MAX_NAME_LENGTH: 40, MAX_PRIVATE_CHATS: 70, };

const state = { accounts: [], publicMessages: [], privateThreads: {}, currentAccountId: null, selectedPrivatePeerId: null, selectedUserId: null, pendingAction: null, monitorPanelEl: null, toastHostEl: null, activitySaveTimer: null, intervalTimer: null, view: "home", searchQuery: "", privateSearchQuery: "", externalDB: null, };
...
function safeJSONParse(value, fallback) { try { if (!value) return fallback; return JSON.parse(value); } catch { return fallback; } }
...
function clampText(value, max = CONFIG.MAX_NAME_LENGTH) { return normalizeText(value).slice(0, max); }

function createId(prefix = "id") { return ${prefix}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}; }

function hashString(str) { let h = 0; const s = String(str || ""); for (let i = 0; i < s.length; i++) { h = (h << 5) - h + s.charCodeAt(i); h |= 0; } return Math.abs(h); }

function colorFromText(text) { const h = hashString(text) % 360; return hsl(${h} 35% 28%); }

function formatTime(ts) { try { return new Date(ts).toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit", }); } catch { return ""; } }

function timeAgo(ts) { const diff = Math.max(0, now() - Number(ts || 0)); const minute = 60 * 1000; const hour = 60 * minute; const day = 24 * hour;
...
function durationLabel(ms) { const totalMinutes = Math.floor(Math.max(0, ms) / 60000); const hours = Math.floor(totalMinutes / 60); const minutes = totalMinutes % 60;
...
function getAccountById(id) { return getAccounts().find((acc) => acc.id === id) || null; }

function getAccountByUsername(username) { const key = normalizeText(username).toLowerCase(); if (!key) return null; return getAccounts().find((acc) => normalizeText(acc.username).toLowerCase() === key) || null; }

function getDisplayName(account) { if (!account) return "مستخدم"; const name = clampText(account.profile?.name || account.username || "مستخدم", CONFIG.MAX_NAME_LENGTH); return name || "مستخدم"; }

function getAvatarInitial(account) { const name = getDisplayName(account); return name ? name[0] : "؟"; }

function getCurrentSession() { return safeJSONParse(localStorage.getItem(KEYS.currentSession), null); }

function isSessionExpired(session) { if (!session || !session.expiresAt) return true; return now() > Number(session.expiresAt); }

function getCurrentAccount() { if (!state.currentAccountId) return null; return getAccountById(state.currentAccountId); }

function isCurrentAccountOnline() { const acc = getCurrentAccount(); if (!acc) return false;
...
function isCurrentAccountFeatured() { const acc = getCurrentAccount(); if (!acc) return false; if (!isCurrentAccountOnline()) return false;
...
function setAvatar(el, account, fallbackLabel = "؟") { if (!el) return;
...
function pruneNotifications(acc) { if (!acc) return; if (!Array.isArray(acc.notifications)) acc.notifications = []; if (acc.notifications.length > CONFIG.MAX_NOTIFICATIONS) { acc.notifications = acc.notifications.slice(-CONFIG.MAX_NOTIFICATIONS); } }

function normalizeThread(thread) { if (!thread || typeof thread !== "object") return null;
...
function prunePrivateThreads() { const cleaned = {}; Object.entries(state.privateThreads || {}).forEach(([key, thread]) => { const t = normalizeThread(thread); if (!t) return;
...
function getThreadKey(a, b) { return [a, b].sort().join("__"); }
...
function ensureCurrentAccount() { const session = getCurrentSession();
...
function createAccount(username, password = "") { const name = clampText(username, CONFIG.MAX_NAME_LENGTH); const pass = String(password || "").trim();
...
function logoutCurrentAccount(showMessage = true) { commitCurrentSession(true); state.selectedPrivatePeerId = null; state.selectedUserId = null;
...
function requireAuth(action) { state.pendingAction = action || null; return canUseCurrentSession(); }
...
function markCurrentNotificationsRead() { const acc = getCurrentAccount(); if (!acc || !Array.isArray(acc.notifications)) return;
...
function getUnreadNotificationCount() { const acc = getCurrentAccount(); if (!acc || !Array.isArray(acc.notifications)) return 0; return acc.notifications.filter((n) => !n.read).length; }

function getMonitorItems() { const acc = getCurrentAccount(); if (!acc || !Array.isArray(acc.notifications)) return []; return [...acc.notifications].sort((a, b) => Number(b.at) - Number(a.at)); }

function openHome() { state.selectedUserId = null; state.selectedPrivatePeerId = null; setView("home"); }

function closeDrawer() { if (!els.menuDrawer) return; els.menuDrawer.classList.add("is-hidden"); els.menuDrawer.setAttribute("aria-hidden", "true"); }

function openDrawer() { if (!els.menuDrawer) return; els.menuDrawer.classList.remove("is-hidden"); els.menuDrawer.setAttribute("aria-hidden", "false"); }

function toggleDrawer() { if (!els.menuDrawer) return; if (els.menuDrawer.classList.contains("is-hidden")) openDrawer(); else closeDrawer(); }
...
function openProfile() { if (!canUseCurrentSession()) return; state.selectedUserId = null; setView("profile"); }
...
function openAccountProfileById(userId) { openUserProfileById(userId); }
...
function renderHomeView() { renderShellState(); renderPublicMessages(); renderOnlineUsers(); renderFeaturedUsers(); renderPrivateChatsList(); renderMonitorPanel(); renderUserSearchResults(); }
...
function getPrivateChatMatches() { const current = getCurrentAccount(); if (!current) return [];

const q = normalizeText(state.privateSearchQuery || "").toLowerCase();
...
function createPrivateSearchBox() { if (!els.privateListPanel) return; if (els.privateSearchInput) return;

const wrap = document.createElement("label");
wrap.className = "search-box";
wrap.setAttribute("for", "privateSearchInput");
wrap.style.marginBottom = "8px";
wrap.style.display = "grid";
wrap.style.gap = "7px";

wrap.innerHTML = `
  <span class="search-label">بحث في الرسائل الخاصة</span>
  <input id="privateSearchInput" type="search" placeholder="ابحث في الأشخاص أو آخر رسالة..." autocomplete="off" />
`;

const titleBlock = els.privateListPanel.querySelector(".panel-head");
if (titleBlock && titleBlock.parentElement === els.privateListPanel) {
  els.privateListPanel.insertBefore(wrap, titleBlock.nextSibling);
} else {
  els.privateListPanel.insertBefore(wrap, els.privateListPanel.firstChild);
}

els.privateSearchInput = wrap.querySelector("#privateSearchInput");
if (els.privateSearchInput) {
  els.privateSearchInput.value = state.privateSearchQuery || "";
  els.privateSearchInput.style.width = "100%";
  els.privateSearchInput.style.borderRadius = "14px";
  els.privateSearchInput.style.border = "1px solid rgba(255, 255, 255, 0.08)";
  els.privateSearchInput.style.background = "linear-gradient(180deg, rgba(255, 255, 255, 0.03), rgba(255, 255, 255, 0.015))";
  els.privateSearchInput.style.color = "var(--text)";
  els.privateSearchInput.style.padding = "11px 12px";
  els.privateSearchInput.style.outline = "none";
  els.privateSearchInput.style.minWidth = "0";
}

}

function renderPrivateChatsList() { if (!els.privateChatsList || !els.privateChatsEmpty) return;

createPrivateSearchBox();
...
function renderUserView() { const target = getAccountById(state.selectedUserId); if (!target) { if (els.userViewTitle) els.userViewTitle.textContent = "ملف المستخدم"; if (els.userViewName) els.userViewName.textContent = "اسم المستخدم"; if (els.userViewStatus) els.userViewStatus.textContent = "المستخدم غير موجود"; if (els.userViewBio) els.userViewBio.textContent = "لا توجد بيانات."; setAvatar(els.userViewAvatar, null, "؟"); return; }
...
function syncUserSearchInputs(value) { const normalized = normalizeText(value || ""); state.searchQuery = normalized; if (els.userSearchInputHome && els.userSearchInputHome.value !== normalized) els.userSearchInputHome.value = normalized; if (els.userSearchInputDrawer && els.userSearchInputDrawer.value !== normalized) els.userSearchInputDrawer.value = normalized; renderUserSearchResults(); }
...
function renderAll() { renderShellState(); renderHomeView(); renderProfileView(); renderPrivateChatsList(); renderPrivateConversation(); renderUserView(); renderMonitorPanel(); renderUserSearchResults(); }
...
function handlePublicSubmit(event) { event.preventDefault(); const text = normalizeText(els.publicMessageInput?.value || ""); if (!text) { showToast("اكتب رسالة أولًا."); return; }
...
function handlePrivateSubmit(event) { event.preventDefault(); const text = normalizeText(els.privateMessageInput?.value || ""); const peerId = state.selectedPrivatePeerId;
...
function handleAppTitleClick() { location.reload(); }

function handlePrivateShortcutClick() { state.selectedPrivatePeerId = null; setView("private"); renderPrivateChatsList(); renderPrivateConversation(); }
...
function createMonitorPanel() { if (!els.menuDrawer) return;
...
function setupIntervals() { if (state.intervalTimer) clearInterval(state.intervalTimer);
...
function tryBindExternalDB() { const db = window.KAREEM3_DB; if (!db || typeof db.init !== "function") return Promise.resolve(null);
...
function ensureDemoPeerForTesting() { const current = getCurrentAccount(); if (!current) return null;
...
function initLocalData() { readStorage(); const guest = ensureCurrentAccount(); ensureDemoPeerForTesting();
...
async function init() { cacheElements(); createMonitorPanel(); initLocalData(); createPrivateSearchBox(); attachEvents(); setupIntervals(); renderAll(); openHome(); await tryBindExternalDB(); if (canUseCurrentSession()) markActivity(); }

function cacheElements() { els.app = $("app"); els.privateShortcutBtn = $("privateShortcutBtn"); els.appTitleBtn = $("appTitleBtn"); els.menuBtn = $("menuBtn"); els.currentUserState = $("currentUserState"); els.profileMonitorBtn = $("profileMonitorBtn"); els.profileMonitorCount = $("profileMonitorCount");
...
els.privateListPanel = els.privateView?.querySelector(".private-list-panel") || null;
...
window.KAREEM3 = { refresh: renderAll, logout: logoutCurrentAccount, openProfile: openProfile, openUserProfileById, openPrivateChat, state: () => ({ currentAccount: getCurrentAccount(), currentSession: getCurrentSession(), unreadNotifications: getUnreadNotificationCount(), view: state.view, }), };
